...
export default function App() {
  ...
  if (!session) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",background:"#F7F6F2",fontFamily:"'Inter',-apple-system,sans-serif",flexDirection:"column",gap:12}}>
      ...
      <button
        onClick={async()=>{const{data,error}=await supabase.auth.signInWithOAuth({provider:"github",options:{redirectTo:window.location.href}});console.log("oauth data:",JSON.stringify(data),"error:",error?.message);if(error)alert("Sign-in error: "+error.message);}}
